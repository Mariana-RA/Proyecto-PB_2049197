...
void Alta()
{
	...
	for (int i = 0; i < alta; i++)
	{
		...
		while (repetido)
		{
			...
			for (int j = 0; j < i; j++)
			{
				if (videojuegos[i].num_art == videojuegos[j].num_art)
				{
					printf("El #item del art%cculo ya est%c en uso. Ingrese otro #item \n", 161, 160);
					repetido = true;
					break;
				}
			}
...
void Lista()
{
	int opcion2,decision2;
	string gender_busq,clasi_busq,consol_busq,año_busq;
	printf("Ingrese la opci%cn que desee: \n1.-B%csqueda por filtros \n2.-Lista completa \n",162, 163);
	scanf_s("%d", &decision2);
	if (decision2 == 1)
	{
		printf("Ingrese la categor%ca que desee buscar en los art%cculos \n", 161, 161);
		printf("%c 1.-G%cnero \n%c 2.-Clasificaci%cn \n%c 3.-Consola \n", 175, 130, 175, 162, 175);
		scanf_s("%d", &opcion2);
		cin.ignore();
		...
		switch (opcion2)
		{
		case 1: //Por el género del art
			printf("Ingresa el g%cnero que desees buscar \n", 130);
			getline(cin, gender_busq);
			for (int i = 0; i < alta; i++)
			{
				if (gender_busq == videojuegos[i].gender_art && videojuegos[i].status != "ELIMINADO")
				{
				...
				else if (gender_busq == videojuegos[i].gender_art && videojuegos[i].status == "ELIMINADO")
				{
					printf("El art%cculo %s est%c eliminado \n", 161, videojuegos[i].num_art.c_str(), 160);
				}
			}
			break;
		case 2: //Por la clasificación del art
			printf("Ingresa la clasificaci%cn que desees buscar \n", 162);
			getline(cin, clasi_busq);
			for (int i = 0; i < alta; i++)
			{
				if (clasi_busq == videojuegos[i].clasi_art && videojuegos[i].status != "ELIMINADO")
				{
					printf("\t ARTICULO #%s \n", videojuegos[i].num_art.c_str());
					printf("NOMBRE: %s \n", videojuegos[i].art_nom.c_str());
					printf("DESCRIPCION: %s \n", videojuegos[i].descrip.c_str());
				...
				else if (clasi_busq == videojuegos[i].clasi_art && videojuegos[i].status == "ELIMINADO")
				{
					printf("El art%cculo %s est%c eliminado \n", 161,videojuegos[i].num_art.c_str(), 160);
				}
			}
			break;
		case 3: //Por consola del art
			printf("Ingresa la consola que desees buscar \n");
			getline(cin, consol_busq);
			for (int i = 0; i < alta; i++)
			{
				if (consol_busq == videojuegos[i].consol_art && videojuegos[i].status != "ELIMINADO")
				{
					printf("\t ARTICULO #%s \n", videojuegos[i].num_art.c_str());
					printf("NOMBRE: %s \n", videojuegos[i].art_nom.c_str());
					printf("DESCRIPCION: %s \n", videojuegos[i].descrip.c_str());
				...
				else if (consol_busq == videojuegos[i].consol_art && videojuegos[i].status == "ELIMINADO")
				{
					printf("El art%cculo %s est%c eliminado \n", 161, videojuegos[i].num_art.c_str(), 160);
				}
			}
			break;
		default:
			printf("OPCION INVALIDA,eliga otra opci%cn \n", 162);
			return Lista();
			break;
		}
	}
